 ...
 */
#define FRAMEBUFFER_PIXEL_FORMAT  2
    ...
    )
{
	...
	if (osdm_plane_pan_buffer(m->framebuffer->fd, offset / m->finfo.line_length) < 0) {
		ALOGE("osdm_plane_pan_buffer failed");
    ...
    )
{
	...
	if (osdm_get_screen_size(&screen_width, &screen_height) != 0) {
		ALOGE("osdm_get_screen_size failed");
		return -errno;
	}
...
#endif
		ALOGE("cannot create main gui osdm plane");
		return -errno;
	}

	if (osdm_plane_per_pixel_alpha_enable(fd) < 0) {
		ALOGE("cannot enable per pixel alpha\n");
		return -1;
	}

	if (osdm_plane_enable(fd) < 0) {
		ALOGE("cannot enable plane fd=%d",fd);
		return -errno;
	}
...
#ifdef USE_OSDM
	ALOGW("finfo.line_length = %d", finfo.line_length);
