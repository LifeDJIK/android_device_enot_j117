    ...
    )
{
    ...
    map  = gcvNULL;
    next = Texture->maps;
    imageformat = gcvUNKNOWN_MIPMAP_IMAGE_FORMAT;
    ...
    for (level = 0; level <= Level; level += 1)
    {
        ...
        if (next == gcvNULL)
        {
            ...
            /* Initialize the gcsMIPMAP structure. */
            next->imageFormat = gcvUNKNOWN_MIPMAP_IMAGE_FORMAT;
    ...
    if (gcmIS_SUCCESS(status))
    {
        ...
        (
            ...
            (map->surface != gcvNULL)   &&
            (
                (map->imageFormat != ((imageFormat == gcvUNKNOWN_MIPMAP_IMAGE_FORMAT) ? imageformat : imageFormat)) ||
                (map->format != Format) ||
                (map->width  != Width)  ||
                (map->height != Height) ||
                (map->depth  != Depth)  ||
    ...
    )
{
    ...
    /* Copy the data. */
    status = gcoHARDWARE_UploadCompressedTexture(
        &map->surface->info,
        Memory,
        offset,
        Width,
    ...
    )
{
    ...
    if (status == gcvSTATUS_NOT_MULTI_PIPE_ALIGNED)
    {
        /* Texture will be reset to be multipipe. */
        status = gcvSTATUS_OK;
    }
    ...
    )
{
    ...
    {
        gctINT level;
        gctINT imageFormat = gcvUNKNOWN_MIPMAP_IMAGE_FORMAT;
        ...
        for (level = 0; level <= MaxLevel; level += 1)
        {
            ...
            else
            {
                ...
                if ((imageFormat != curr->imageFormat) ||
                    (format != curr->format) ||
                    (width  != curr->width)  ||
                    (height != curr->height) ||
                    (depth  != curr->depth)  ||
    ...
    )
{
    ...
    if (Sampler >= 0)
    {
        ...
        if (Texture->maps != gcvNULL)
        {
            ...
            samplerInfo.roundUV     = (Texture->Info.magFilter == gcvTEXTURE_POINT && samplerInfo.width <= 32 && samplerInfo.height <= 32 ) ? 0:1;
        ...
        else
        {
            ...
            samplerInfo.roundUV     = (Texture->Info.magFilter == gcvTEXTURE_POINT ) ? 0:1;
            samplerInfo.hasTileStatus = 0;
            samplerInfo.tileStatusAddr = 0;
            samplerInfo.tileStatusClearValue = 0;
        }
