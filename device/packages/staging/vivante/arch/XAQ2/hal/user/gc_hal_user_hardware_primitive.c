    ...
    )
{
    ...
    */
    ...
    if (Command == gcv2D_STRETCH)
    {
        ...
        destConfig |= ((((gctUINT32) (0)) & ~(((gctUINT32) (((gctUINT32) ((((1 ? 24:24) - (0 ? 24:24) + 1) == 32) ? ~0 : (~(~0 << ((1 ? 24:24) - (0 ? 24:24) + 1))))))) << (0 ? 24:24))) | (((gctUINT32) ((gctUINT32) (State->gdiStretch ? 0x1 : 0x0) & ((gctUINT32) ((((1 ? 24:24) - (0 ? 24:24) + 1) == 32) ? ~0 : (~(~0 << ((1 ? 24:24) - (0 ? 24:24) + 1))))))) << (0 ? 24:24)));
    }
...
static int _SplitRectangle(
    gctUINT32_PTR Memory, gcsRECT_PTR SrcRect, gcsRECT_PTR DestRect, gcsRECT_PTR ClipRect, gctBOOL HorizontalMirror)
{
    gctINT32 n, size = 0;
    gcsRECT srcRect, dstRect;
    gctINT32 sRight, sLeft, dRight, dLeft;

    dLeft = dstRect.left = gcmMAX(DestRect->left, ClipRect->left);
    dstRect.top = gcmMAX(DestRect->top, ClipRect->top);
    dRight = dstRect.right = gcmMIN(DestRect->right, ClipRect->right);
    dstRect.bottom = gcmMIN(DestRect->bottom, ClipRect->bottom);

    sLeft = srcRect.left = SrcRect->left;
    srcRect.top = SrcRect->top;
    sRight = srcRect.right = dstRect.right - dstRect.left + srcRect.left;
    ...
    )
{
    ...
    do
    {
        ...
        if (hardware->hw2DEngine && !hardware->sw2DEngine)
        {
            ...
            gctBOOL split = (Command == gcv2D_BLT) && (!hardware->hw2DDEEnhance1);
                    ...
                    if (split)
                    {
                        ...
                        for (i = 0; i < DestRectCount; i++)
                        {
                            rectNum += (gcmALIGN(DestRect[i].right, SPLIT_COLUMN_WIDTH) -
                                (DestRect[i].left & SPLIT_COLUMN_WIDTH_MASK)) / SPLIT_COLUMN_WIDTH;
                        }
