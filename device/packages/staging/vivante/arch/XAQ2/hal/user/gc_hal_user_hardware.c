    ...
    )
{
    ...
    */
    ...
    hardware->varyingsCount          = iface.u.QueryChipIdentity.varyingsCount;
    ...
    )
{
    ...
    /* Program TileFiller. */
    tileCount = gcmALIGN(Surface->node.size / 64, 256);
    ...
    )
{
    ...
    ** cache lines are pushed out. */
    ...
    if (Decompress && hardware->colorStates.dirty)
    {
        ...
        {
            if (gcoHARDWARE_IsFeatureAvailable(gcvFEATURE_TILE_FILLER) &&
                gcoHARDWARE_IsFeatureAvailable(gcvFEATURE_FC_FLUSH_STALL) &&
                (Surface->samples.x == 1) && (Surface->samples.y == 1))
            {
                ...
                /* Fast clear fill. */
                gcmONERROR(gcoHARDWARE_FillFromTileStatus(Surface));
    ...
    )
{
    ...
    /* Determine which Dither Path to go. */
