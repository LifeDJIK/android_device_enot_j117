    ...
    )
{
    ...
    if (hnd->pid != getpid())
    {
        ...
        if (hnd->resolveSurface != 0)
        {
            ...
            if (gcmALIGN(hnd->width, 4) & 0x0F)
            {
                /* Align tiled surface to 16 pixel alignment for resolve,
                 * which is already done while allocation. We need to make
                 * corresponding alignment here in client process. */
                scaledWidth = gcmALIGN(hnd->width, 16);
            }
            else
            {
                scaledWidth = (gctUINT) hnd->width;
            }
