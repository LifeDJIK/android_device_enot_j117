    ...
    )
{
    ...
    case ANDROID_HAL_PIXEL_FORMAT_YCbCr_422_SP:
        /* YUV 422 semi planner: NV16 */
        *HalFormat = gcvSURF_NV16;
        break;

    case ANDROID_HAL_PIXEL_FORMAT_YCrCb_422_SP:
        /* YVU 422 semi planner: NV61 */
        *HalFormat = gcvSURF_NV61;
        break;
    ...
    )
{
    ...
    if (m->framebuffer == NULL)
    {
        ...
        ** and forever. */
        ...
        if (err < 0)
        {
            ALOGE("%s: ------> mapFrameBufferLocked FAILED <------\n", __func__);
            return err;
        }
    }
    ...
    {
        ALOGE("%s: ------> We ran out of buffers (%d, %d, %d) <------\n", __func__, numBuffers, bufferMask, ((1LU << numBuffers) - 1));
    ...
    ALOGE("Allocated framebuffer handle=%p, vaddr=%p, paddr=%p",
    ...
    *Stride = m->finfo.line_length / (m->info.bits_per_pixel / 8);
    ...
    ALOGE("%s: ------> OK for gcoHAL_SetHardwareType <------\n", __func__);

    /* Get base address. */
    gcmONERROR(
        gcoOS_GetBaseAddress(gcvNULL, &baseAddress));

    ALOGE("%s: ------> OK for gcoOS_GetBaseAddress <------\n", __func__);
    ...
    ALOGE("%s: ------> OK for gcoSURF_ConstructWrapper <------\n", __func__);

    gcmONERROR(
        _ConvertAndroid2HALFormat(Format,
                                  &format));

    ALOGE("%s: ------> OK for _ConvertAndroid2HALFormat <------\n", __func__);
    ...
    ALOGE("%s: ------> OK for gcoSURF_SetBuffer <------\n", __func__);
    ...
    ALOGE("%s: ------> OK for gcoSURF_SetWindow <------\n", __func__);
    ...
    )
{
    ...
    if (Usage & GRALLOC_USAGE_HW_RENDER)
    {
        ...
        {
            /* Align tiled surface to 16 pixel alignment for resolve. */
        ...
        ALOGV("resolve buffer resolveSurface=%p, "
             "resolveNode=%p, "
             "resolvevidnode=%p, "
             "resolveBytes=%d, "
             "resolvePool=%d, "
...
#endif
    {
        ...
        ALOGV("allocated buffer surface=%p, "
             "node=%p, "
             "vidNode=%p, "
             "pool=%d, "
             "bytes=%d, "
    ...
    ALOGV("allocated buffer handle=%p, "
         "width=%d, height=%d, "
         "formatRequested=%d, "
         "usage=0x%08X, "
         "stride=%d pixels",
