    ...
    )
{
    ...
    /* EZ should be turned off if the shader re-programs depth. */
    ez = !Context->program->hints->psHasFragDepthOut;

#if gldPATCHES
    /* Query EZ support from patching. */
    ez &= glshQueryPatchEZ(Context);
#endif

    gcmONERROR(gco3D_SetAllEarlyDepthModes(Context->engine, !ez));
        ...
        else
        {
            ...
            if (texture->source != gcvNULL)
            {
                ...
                if (texWidth != srcWidth)
                {
                    ...
                    /* Upload texture. */
                    gcmVERIFY_OK(gcoTEXTURE_Upload(
                        texture->texture,
                        0,
                        texture->width,
                        texture->height,
                                           ...
                                           const GLvoid * Indices)
{
    ...
    glmENTER4(glmARGENUM, Mode, glmARGINT, Count, glmARGENUM, Type, glmARGPTR, Indices)
    {
        if (context->vertexArrayObject == gcvNULL)
        {
        ...
        glmBATCH(context,
                 break,
                 break,
                 (glshBatchDrawElements(context, Mode, Count, Type, Indices)));
#endif

        /* Call the funciton. */
        glshDrawElements(context, Mode, Count, Type, Indices, elementArrayBuffer);
    }
    glmLEAVE()
#endif
}
                        ...
                        IN GLBuffer Buffer)
{
...
#if gldFBO_DATABASE
    /* Check if we have a current framebuffer and we are not playing back. */
    if (Context->dbEnable
        &&
        (Context->framebuffer != gcvNULL)
        &&
    ...
    gcmVERIFY_OK(glshCheckDependency(Context));
        ...
        if (!glshPatchDraw(Context, Mode, Count, Type, Indices))
        {
            /* Nothing to draw now. */
            return GL_NO_ERROR;
        }
    ...
    if (Context->patchStrip)
    {
        ...
        {
            indexBuffer = (Buffer != gcvNULL) ? Buffer->index : gcvNULL;
        }
    }
    else
#endif
    ...
    if ((Indices == gcvNULL) && (Buffer == gcvNULL))
    {
        gcmTRACE_ZONE(gcvLEVEL_WARNING, gldZONE_DRAW,
    ...
    if (Context->cullEnable && (Context->cullMode == GL_FRONT_AND_BACK)
        &&
   ...
   /* Walk all attributes. */
    ...
    {
        if ((vertexArrayGL[i].buffer != gcvNULL)
            &&
            (vertexArrayGL[i].buffer == Context->arrayBuffer)
            &&
            (vertexArray[i].stream != vertexArrayGL[i].buffer->stream)
            )
        {
            vertexArray[i].stream = vertexArrayGL[i].buffer->stream;
        }
...
OnError:
    ...
    do
    {
        ...
        /* Draw primitives. */
        gcmERR_BREAK(_DrawPrimitives(Context,
                                     primitiveType,
                                     0,
                                     primitiveCount,
                                     gcvTRUE));
                                         ...
                                         GLsizei Count)
{
    ...
    {
#if gcdDUMP_API
        gcmDUMP_API("${ES20 glDrawArrays 0x%08X 0x%08X 0x%08X", Mode, First, Count);
        ...
        glmBATCH(context,
                 break,
                 break,
                 (glshBatchDrawArrays(context, Mode, First, Count)));
#endif

        glshDrawArrays(context, Mode, First, Count);
    }
    glmLEAVE()
#endif
}
                      ...
                      IN GLsizei Count)
{
    ...
    for (i = enable = 0; i < (gctINT) Context->maxAttributes; ++i)
    {
        ...
        if ((link < Context->maxAttributes) && ((Context->program->attributeEnable >> link) & 1))
        {
            enable |= (1 << i);
        }
    }
    ...
    for (i = 0; i < gldVERTEX_ELEMENT_COUNT; ++i)
    {
        if ((vertexArrayGL[i].buffer != gcvNULL) && (vertexArray[i].stream != vertexArrayGL[i].buffer->stream))
        {
            vertexArray[i].stream = vertexArrayGL[i].buffer->stream;
        }
    }
    ...
    lineLoopPatch = (Mode == GL_LINE_LOOP) && (primitiveType != gcvPRIMITIVE_LINE_LOOP);
