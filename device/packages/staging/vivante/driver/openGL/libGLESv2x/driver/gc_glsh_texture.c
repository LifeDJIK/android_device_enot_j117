    ...
    )
{
    ...
    Context->default2D.anisotropy = 1;
    ...
    Context->defaultCube.anisotropy = 1;
    ...
    Context->default3D.anisotropy = 1;
    ...
    Context->defaultExternal.anisotropy = 1;
    ...
    )
{
    ...
    do
    {
        ...
        Texture->states.anisoFilter = Texture->anisotropy;
...
#if gldPATCHES
        /* Do batching. */
        glmBATCH(context, break, break, (glshBatchActiveTexture(context, texture)));
#endif

        glshActiveTexture(context, texture);
    }
    glmLEAVE()
#endif
}

GLenum glshActiveTexture(IN GLContext Context,
                         IN GLenum Texture)
{
    GLint   unit;
...
GLTexture glshNewTexture(IN GLContext Context, IN GLuint Name)
{
    GLTexture   texture;
    gceSTATUS   status;
    ...
    if (gcmIS_ERROR(gcoOS_Allocate(gcvNULL, gcmSIZEOF(struct _GLTexture), &pointer)))
    {
        /* Out of memory error. */
    ...
    /* Initialize memory. */
    gcmVERIFY_OK(gcoOS_ZeroMemory(pointer, sizeof(struct _GLTexture)));

    texture = pointer;

    /* Create a new object. */
    if (!_glshInsertObject(&Context->textureObjects, &texture->object, GLObject_Texture, Name))
    {
        /* Roll back. */
        gcmVERIFY_OK(gcmOS_SAFE_FREE(gcvNULL, texture));
    ...
    gcoOS_ZeroMemory(texture->direct.texture, gcmSIZEOF(gcoSURF) * 16);
...
#if gldPATCHES
    /* Initialize the batching. */
    gcoOS_ZeroMemory(&texture->batchedState, gcmSIZEOF(texture->batchedState));
    texture->batchedState.texture = texture->object.name;
#endif
    ...
    )
{
    ...
    for (i = 0; i < n; ++i)
    {
        ...
        texture = glshNewTexture(context, 0);
        if (texture == gcvNULL)
        {
            break;
        }
...
#if gldPATCHES
        /* Synchronize with batching. */
        glmBATCH(context, break, glmBATCH_NOP, (glshBatchSync(context, gcvFALSE)));
#endif
    ...
    )
{
    ...
    glmENTER2(glmARGENUM, target, glmARGUINT, texture)
    {
        ...
        glmBATCH(context,
                 break,
                 break,
                 (glshBatchTextureBinding(context, target, texture)));
#endif

        glshBindTexture(context, target, texture);
    }
    glmLEAVE()
#endif
}

GLenum glshBindTexture(IN GLContext Context,
                       IN GLenum Target,
                       IN GLuint Texture)
{
    GLTexture   object;
    ...
    else
    {
        ...
        if (object == gcvNULL)
        {
            object = glshNewTexture(Context, Texture);
            if (object == gcvNULL)
            {
        ...
        if ((Target == GL_TEXTURE_EXTERNAL_OES) && (object->target == 0))
        {
    ...
    )
{
    ...
    if (pixels != gcvNULL)
    {
        ...
        if(texture->image != gcvNULL)
        {
             ...
             _glshSetImageSrc(texture->image,mipmap);
        }
    ...
    )
{
    ...
    do
    {
        ...
        /* Bound to default texture? */
        if (Texture->object.name == 0)
        {
            /* Hack to let the framebuffer binding go to the default texture. */
            Context->copyTexShaderInside = gcvTRUE;
        }

        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, Texture->object.name, 0);
        Context->copyTexShaderInside = gcvFALSE;
    ...
    )
{
    ...
    glmENTER3(glmARGENUM, target, glmARGENUM, pname, glmARGINT, param)
    {
        ...
        glmBATCH(context,
                 break,
                 break,
                 (glshBatchTextureParameter(context, target, pname, param)));
#endif
    ...
    )
{
    ...
    glmENTER3(glmARGENUM, target, glmARGENUM, pname, glmARGFLOAT, param)
    {
        ...
        glmBATCH(context,
                 break,
                 break,
                 (glshBatchTextureParameter(context, target, pname, (GLint)param)));
#endif
    ...
    )
{
    ...
    glmENTER3(glmARGENUM, target, glmARGENUM, pname, glmARGPTR, params)
    {
        ...
        glmBATCH(context,
                 break,
                 break,
                 (glshBatchTextureParameter(context, target, pname, params[0])));
#endif
    ...
    )
{
    ...
    glmENTER3(glmARGENUM, target, glmARGENUM, pname, glmARGPTR, params)
    {
        ...
        glmBATCH(context,
                 break,
                 break,
                 (glshBatchTextureParameter(context, target, pname, (GLint)params[0])));
#endif
    ...
    )
{
    ...
    /* Don't use shader path if resolve can handle the copy. */
    unalignedCopy = ((x & 0xF) || (y & 0x8) || (width & 0xF) || (height & 0x8));
    ...
    else
    {
        ...
        if ( gcdCOPY_TEX_SHADER
         ...
         && gcoHAL_IsFeatureAvailable(gcvNULL, gcvFEATURE_SUPERTILED_TEXTURE))
        {
    ...
    if(texture->image != gcvNULL)
    {
        _glshSetImageSrc(texture->image,surface);
    }
    ...
    )
{
    ...
    while (levels-- > 0)
    {
        ...
        if ((imageSize > 0) && (data != gcvNULL))
        {
            ...
            case GL_COMPRESSED_RGBA_PVRTC_4BPPV1_IMG:
            {
                /* Compute uncompressed texture size. */
                GLsizei uncompressedSize = width * height * 4;
                ...
                status = gcoOS_Allocate(gcvNULL, uncompressedSize,
                                        (gctPOINTER *) &pixels);
                ...
                glshDecompressPVR(internalformat, width, height, data, pixels);
            }
#endif
    ...
    )
{
    ...
    do
    {
#if defined(ANDROID) && (ANDROID_SDK_VERSIOIN >= 7)
        /* Reset private handle. */
        Texture->privHandle = gcvNULL;
    ...
    )
{
    ...
    else
    {
        ...
        */
        ...
        case gcvSURF_NV16:
        case gcvSURF_NV61:
            /* TODO: NV16/NV61 is not directly supported by 3D. */
            texture->direct.textureFormat = gcvSURF_YUY2;
            yuvFormat = gcvTRUE;
            break;
    ...
    surface = (image->srcSurface == gcvNULL) ? image->surface : image->srcSurface;
    ...
    )
    {
        ...
        if (type != gcvSURF_TEXTURE)
        {
            ...
            texture->privHandle = image->u.androidNativeBuffer.privHandle;
