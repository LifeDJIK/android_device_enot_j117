...
#define gldSPLIT_OBJECT_NAME_SPACE    1

#if gldSPLIT_OBJECT_NAME_SPACE
static gcsATOM_PTR         gcContextID = gcvNULL;
static gcsATOM_PTR         gcContextCount = gcvNULL;
#endif
    ...
    )
{
...
#if gldSPLIT_OBJECT_NAME_SPACE
    if(gcContextID == gcvNULL)
    {
        if(gcmIS_ERROR(gcoOS_AtomConstruct(gcvNULL, &gcContextID)))
        {
             gcContextID = gcvNULL;
             return;
        }
    ...
    gcoOS_AtomIncrement(gcvNULL,gcContextID,&contextID);
    
    Context->bufferObjects.lastObject = contextID << 20; 
#endif
}
