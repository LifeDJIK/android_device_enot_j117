...
static gceSTATUS glshNewDatabase(IN glsDATABASE_CONTAINER * Container,
                                 IN GLFramebuffer Owner,
                                 OUT glsDATABASE_PTR * Database)
{
    gceSTATUS       status;
    glsDATABASE_PTR db = gcvNULL;

    gcmHEADER_ARG("Container=%p Owner=%p", Container, Owner);

    if (Container->freeDBCount == 0)
    {
    ...
    else
    {
        ...
        Container->freeDBCount -= 1;

        /* Get database from the free list. */
        db = Container->freeDB[Container->freeDBCount];
    }
...
static gceSTATUS glshSelectDatabase(IN glsDATABASE_CONTAINER * Container,
                                    IN GLFramebuffer Owner,
                                    OUT glsDATABASE_PTR * Database)
{
    gceSTATUS   status;
    ...
    /* If there is no current database or its reference is not zero, create
     * a new database. */
    if ((Container->currentDB == gcvNULL)
        ||
        (Container->currentDB->reference != 0)
        )
    {
        /* Create a new database. */
        gcmONERROR(glshNewDatabase(Container, Owner, &Container->currentDB));
    }

    /* Return the current database pointer. */
    *Database = Container->currentDB;
...
static gceSTATUS glshDeleteDatabase(IN glsDATABASE_PTR Database)
{
    glsDATABASE_CONTAINER * container;
    gceSTATUS               status;

    gcmHEADER_ARG("Database=%p", Database);

    /* Get pointer to owner. */
    container = Database->owner;
    ...
    if (container->currentDB == Database)
    {
        /* Remove the current database. */
        container->currentDB = gcvNULL;
    }

    /* Check if the FBO's free list is full. */
    if (container->freeDBCount < gcmCOUNTOF(container->freeDB))
    {
        /* Store the database in the next available free slot. */
        container->freeDB[container->freeDBCount] = Database;
...
static gceSTATUS glshNewRecord(IN glsDATABASE_PTR Database,
                               OUT glsDATABASE_RECORD_PTR * Record)
{
    gceSTATUS               status;
    glsDATABASE_CONTAINER * container;
    ...
    /* Get pointer to the container owing this database. */
    container = Database->owner;

    if (container->freeRecordCount == 0)
    {
    ...
    else
    {
        ...
        container->freeRecordCount -= 1;

        /* Get record from the free list. */
        record = container->freeRecord[container->freeRecordCount];
    }
...
static gceSTATUS glshDeleteRecord(IN glsDATABASE_PTR Database)
{
    gceSTATUS               status;
    glsDATABASE_CONTAINER * container;
    glsDATABASE_RECORD_PTR  record;
    ...
    /* Get the pointer to the container owning the database. */
    container = Database->owner;
    ...
    if (container->freeRecordCount < gcmCOUNTOF(container->freeRecord))
    {
        /* Add the record to the free list. */
        container->freeRecord[container->freeRecordCount] = record;
...
static gceSTATUS glshAddDependency(IN glsDATABASE_PTR Database,
                                   IN glsDATABASE_PTR Dependency)
{
    gceSTATUS                   status;
...
static gceSTATUS glshRemoveReference(IN glsDATABASE_PTR Database)
{
    gceSTATUS   status;
...
static gceSTATUS glshRemoveDependencies(IN glsDATABASE_PTR Database)
{
...
static void glshGetStates(IN GLContext Context,
                          IN glsDATABASE_STATES * States)
{
...
static gceSTATUS glshAddDatabase(IN glsDATABASE_PTR Database,
                                 IN gctSIZE_T Bytes,
...
static gceSTATUS glshInitialStates(IN GLContext Context,
                                   IN glsDATABASE_PTR Database)
{
    gceSTATUS                       status;
...
static gceSTATUS glshAddScissors(IN GLContext Context,
                                 IN glsDATABASE_PTR Database)
{
    gceSTATUS               status;
...
static gceSTATUS glshAddPolygon(IN GLContext Context,
                                IN glsDATABASE_PTR Database)
{
    gceSTATUS               status;
    ...
    else
    {
        ...
        {
            /* Add the record to the database. */
            gcmONERROR(glshAddDatabase(Database, gcmSIZEOF(polygon), &polygon));
...
static gceSTATUS glshAddBlending(IN GLContext Context,
                                 IN glsDATABASE_PTR Database)
{
    gceSTATUS           status;
...
static gceSTATUS glshAddPixel(IN GLContext Context,
                              IN glsDATABASE_PTR Database)
{
    gceSTATUS           status;
    ...
    else
    {
        ...
        {
            /* Add the record to the database. */
            gcmONERROR(glshAddDatabase(Database, gcmSIZEOF(pixel), &pixel));
...
static gceSTATUS glshAddDepth(IN GLContext Context,
                              IN glsDATABASE_PTR Database)
{
    gceSTATUS           status;
    ...
    else
    {
        ...
        {
            /* Add the record to the database. */
            gcmONERROR(glshAddDatabase(Database, gcmSIZEOF(depth), &depth));
...
static gceSTATUS glshAddStencil(IN GLContext Context,
                                IN glsDATABASE_PTR Database)
{
    gceSTATUS               status;
    ...
    else
    {
        ...
        {
            /* Add the record to the database. */
            gcmONERROR(glshAddDatabase(Database, gcmSIZEOF(stencil), &stencil));
...
static gceSTATUS glshNewProgramArray(OUT glsDATABASE_PROGRAM_ARRAY_PTR * Array)
{
    gceSTATUS                       status;
...
static gceSTATUS glshAddProgram(IN GLContext Context,
                                IN glsDATABASE_PTR Database)
{
    gceSTATUS                       status;
    GLProgram                       program;
    ...
    if (Database->currentProgram != program)
    {
        ...
        gcmONERROR(glshAddDatabase(Database, gcmSIZEOF(record), &record));
    ...
    for (i = 0; i < program->uniformCount; i++)
    {
        ...
        if (!known || program->uniforms[i].dbDirty)
        {
            ...
            gcmONERROR(glshAddDatabase(Database, gcmSIZEOF(uniform), &uniform));
...
static gceSTATUS glshNewTextureArray(OUT glsDATABASE_TEXTURE_ARRAY_PTR * Array)
{
    gceSTATUS                       status;
...
static gceSTATUS glshAddTexture(IN GLContext Context,
                                IN glsDATABASE_PTR Database,
                                IN GLTexture Texture,
                                IN GLuint Sampler)
{
    gceSTATUS                       status;
    GLProgram                       program;
    ...
    if (Texture != gcvNULL)
    {
        ...
        if (!known || (Texture->savedAnisoFilter != Texture->anisotropy))
        {
            /* Copy the changes. */
            record.flags      |= GLDB_TEXTURE_ANISO_FILTER;
            record.anisoFilter = Texture->anisotropy;
        }
        ...
        if ((Texture->owner != gcvNULL)
            &&
            (Texture->owner->database.currentDB != gcvNULL)
            )
        {
            /* Add the dependency. */
            gcmONERROR(glshAddDependency(Database,
                                         Texture->owner->database.currentDB));
        }
    }
    ...
    if (record.flags != 0)
    {
        ...
        if (Texture != gcvNULL)
        {
            ...
            Texture->savedAnisoFilter = Texture->anisotropy;
...
static gceSTATUS glshAddTextures(IN GLContext Context,
                                 IN glsDATABASE_PTR Database)
{
    gceSTATUS       status;
    GLProgram       program;
...
static gceSTATUS glshDeltaStates(IN GLContext Context,
                                 IN glsDATABASE_PTR Database)
{
    gceSTATUS   status;
...
gceSTATUS glshAddClear(IN GLContext Context,
                       IN GLbitfield Mask)
{
    gceSTATUS           status;
    glsDATABASE_PTR     db;
    ...
    gcmONERROR(glshSelectDatabase((Context->framebuffer == gcvNULL)
                                  ? &Context->database
                                  : &Context->framebuffer->database,
                                  Context->framebuffer,
                                  &db));
    ...
    if (!Context->scissorEnable && (db->first != gcvNULL))
    {
        ...
        {
            /* Check if there is color buffer. */
            ...
            /* Check if there is a depth buffer. */
            else if (Context->depth != gcvNULL)
            {
                /* Dispatch on the depth buffer format. */
                if (Context->depthFormat == gcvSURF_D24S8)
                {
                    /* If we are clearing the depth and stencil buffer, we can
                     * empty the database. */
                    empty = (((Mask & GL_DEPTH_BUFFER_BIT)   &&
    ...
    if (db->first != gcvNULL)
    {
        ...
        /* Add the record to the database. */
        gcmONERROR(glshAddDatabase(db, gcmSIZEOF(clear), &clear));
    }
    else
    {
...
gceSTATUS glshAddDrawElements(IN GLContext Context,
                              IN GLenum Mode,
                              IN GLsizei Count,
                              IN GLenum Type,
                              IN const GLvoid * Indices)
{
    ...
    gcmONERROR(glshAddDatabase(db, gcmSIZEOF(draw), &draw));

    /* Check if we need to copy the element data. */
    if (draw.elementBytes > 0)
    {
        /* Add the element data to the database. */
        gcmONERROR(glshAddDatabase(db, draw.elementBytes, Indices));
    }
...
gceSTATUS glshAddDrawArrays(IN GLContext Context,
                            IN GLenum Mode,
                            IN GLint First,
                            IN GLsizei Count)
{
    ...
    for (i = 0; i < 16; i++)
    {
        if (draw.vertices[i].bytes > 0)
        {
            ...
            /* Add the vertex data to the database. */
            gcmONERROR(glshAddDatabase(db, draw.vertices[i].bytes, data));
        }
    }
...
gceSTATUS glshCleanDatabase(IN glsDATABASE_PTR Database)
{
    gceSTATUS                       status;
    glsDATABASE_PROGRAM_ARRAY_PTR   programArray;
    glsDATABASE_TEXTURE_ARRAY_PTR   textureArray;
    ...
    {
        gcmTRACE_ZONE(gcvLEVEL_INFO, gldZONE_DATABASE,
                      "Cleaning database %p (FBO %u)\n",
                      Database,
                      Database->fbOwner ? Database->fbOwner->object.name : 0);
    }
...
gceSTATUS glshRemoveDatabase(IN glsDATABASE_CONTAINER * Container)
{
    gceSTATUS   status;
    gctINT      i;

    gcmHEADER_ARG("Container=%p", Container);

    /* Process all databases. */
    while (Container->listDB != gcvNULL)
    {
        /* Clean the database. */
        gcmONERROR(glshCleanDatabase(Container->listDB));

        /* Delete the database. */
        gcmONERROR(glshDeleteDatabase(Container->listDB));
    }

    /* Process all databases in the free list. */
    for (i = 0; i < Container->freeDBCount; i++)
    {
        /* Free the database structure. */
        gcmONERROR(gcoOS_Free(gcvNULL, Container->freeDB[i]));
    }

    /* No more free databases. */
    Container->freeDBCount = 0;

    /* Process all records in the free list. */
    for (i = 0; i < Container->freeRecordCount; i++)
    {
        /* Free the record array. */
        gcmONERROR(gcoOS_Free(gcvNULL, Container->freeRecord[i]));
    }

    /* No more free records. */
    Container->freeRecordCount = 0;
...
static gceSTATUS glshGetRecord(IN glsDATABASE_PTR Database,
                               IN gctSIZE_T MoveBytes,
                               OUT gctPOINTER * Location)
{
    gceSTATUS   status;
...
static gctSIZE_T glshPlayClear(IN GLContext Context,
                               IN glsDATABASE_CLEAR * Clear)
{
...
static gctSIZE_T glshPlayScissors(IN GLContext Context,
                                  IN glsDATABASE_SCISSORS * Scissors,
                                  IN gctBOOL AllStates)
{
...
static gctSIZE_T glshPlayPolygon(IN GLContext Context,
                                 IN glsDATABASE_POLYGON * Polygon,
                                 IN gctBOOL AllStates)
{
...
static gctSIZE_T glshPlayBlend(IN GLContext Context,
                               IN glsDATABASE_BLEND * Blend,
                               IN gctBOOL AllStates)
{
...
static gctSIZE_T glshPlayPixel(IN GLContext Context,
                               IN glsDATABASE_PIXEL * Pixel,
                               IN gctBOOL AllStates)
{
...
static gctSIZE_T glshPlayDepth(IN GLContext Context,
                               IN glsDATABASE_DEPTH * Depth,
                               IN gctBOOL AllStates)
{
...
static gctSIZE_T glshPlayStencil(IN GLContext Context,
                                 IN glsDATABASE_STENCIL * Stencil,
                                 IN gctBOOL AllStates)
{
...
static void glshPlayStates(IN GLContext Context,
                           IN glsDATABASE_STATES * States,
                           IN gctBOOL AllStates)
{
...
static gctSIZE_T glshPlayProgram(IN GLContext Context,
                                 IN glsDATABASE_PROGRAM * Program,
                                 IN GLProgram * ModifiedList)
{
    GLProgram   program;
    gctINT      i;
    GLUniform   uniform;
...
static gctSIZE_T glshPlayUniform(IN GLContext Context,
                                 IN glsDATABASE_UNIFORM * Uniform,
                                 IN glsDATABASE_PTR Database)
{
    gceSTATUS   status;
    GLProgram   program;
...
static gctSIZE_T glshPlayTexture(IN GLContext Context,
                                 IN glsDATABASE_TEXTURE * Texture,
                                 IN GLTexture * ModifiedList)
{
    GLProgram   program;
    GLTexture   texture;
...
static gctSIZE_T glshPlayDrawElements(IN GLContext Context,
                                      IN glsDATABASE_DRAW_ELEMENTS * Elements,
                                      IN glsDATABASE_PTR Database)
{
    gceSTATUS           status;
    gctINT              i;
...
static gctSIZE_T glshPlayDrawArrays(IN GLContext Context,
                                    IN glsDATABASE_DRAW_ARRAYS * Arrays,
                                    IN glsDATABASE_PTR Database)
{
    gceSTATUS       status;
    gctINT          i;
...
static void glshResetTextures(IN GLContext Context,
                              IN GLTexture TextureList)
{
    GLTexture   texture;
         ...
         )
    {
        ...
        if (texture->anisotropy!= texture->savedAnisoFilter)
        {
...
static void glshResetPrograms(IN GLContext Context,
                              IN GLProgram ModifiedList)
{
    GLProgram   program;
    gctINT      i;
...
gceSTATUS glshPlayDatabase(GLContext Context, glsDATABASE_PTR Database)
{
    ...
    /* Get framebuffer owner. */
    framebufferName = ((Database->fbOwner == gcvNULL)
                       ? 0
                       : Database->fbOwner->object.name);

    gcmTRACE_ZONE(gcvLEVEL_INFO, gldZONE_DATABASE,
                  "Playing back database %p from FBO %u",
                  Database, framebufferName);
        ...
        )
    {
        ...
        if (Database->first != gcvNULL)
        {
            ...
            /* Empty the pipe when done. */
            gcmONERROR(gco3D_Semaphore(gcvNULL,
                                       gcvWHERE_COMMAND,
                                       gcvWHERE_PIXEL,
...
gceSTATUS glshCheckDependency(IN GLContext Context)
{
    gceSTATUS   status;
    GLProgram   program;
    gctINT      samplers;
    ...
    for (i = 0; i < samplers; i++)
    {
            ...
            )
        {
            ...
            gcmONERROR(glshPlayDatabase(Context,
                                        texture->owner->database.currentDB));
        }
    }
