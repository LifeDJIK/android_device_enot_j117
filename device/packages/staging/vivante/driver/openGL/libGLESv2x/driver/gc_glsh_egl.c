    ...
    )
{
    ...
    gcoOS_ZeroMemory(pointer, gcmSIZEOF(struct _GLContext));
    ...
    /* Adjust z far is false by default. */
    context->adjustZFar = gcvFALSE;
    context->zFarValue  = 1.01f;
...
#if gldPATCHES
    gcoOS_ZeroMemory(&context->patchInfo, gcmSIZEOF(context->patchInfo));
#endif
...
#if gcdUSE_WCLIP_PATCH
        {
            ...
            WClipGameList[] = {
                {"no.agens.vikersund", 0.7f},
				{"com.bradesco.link237race", 1.0f},
				{"com.GOGN.EFAndroid", 0.7f},
            };

            for (i = 0; i < gcmCOUNTOF(WClipGameList); i++)
            {
                if (gcoOS_DetectProcessByName(WClipGameList[i].name))
                {
                    gco3D_SetWPlaneLimitF(context->engine, WClipGameList[i].wClip);
                    gco3D_SetWClipEnable(context->engine, gcvTRUE);
                    break;
                }
            }
        }
...
#endif
    gcmONERROR(gcoHAL_Get3DEngine(gcvNULL, &currentEngine));
    gcmONERROR(gcoHAL_Get3DHardware(gcvNULL, &currentHW));
    context->engine = currentEngine;
    context->hw = currentHW;
    ...
    )
{
    ...
    if (Context->dbEnable)
    {
        ...
        if (gcmIS_ERROR(glshPlayDatabase(Context, Context->database.currentDB)))
        {
    ...
    )
{
    ...
    */
    ...
    i = gldBLOB_CACHE_MD5_SIZE;
    while (*versionString != '$' && i < gldBLOB_CACHE_KEY_SIZE)
    {
    ...
    while (i < gldBLOB_CACHE_KEY_SIZE)
    {
        DigestMD5[i++] = 0;
    }
}
    ...
    )
{
    ...
    gctSIZE_T allocatedBufferSize = gldBLOB_CACHE_MAX_BINARY_SIZE;
    ...
    )
{
    ...
    do
    {
        ...
        gcmASSERT(bufferSize < gldBLOB_CACHE_MAX_BINARY_SIZE);
        ...
        /* Cache the binary. */
        veglSetBlobCache(BlobCacheKey, gldBLOB_CACHE_KEY_SIZE, buffer, bufferSize);
...
void _glshSetImageSrc(void* eglImage, gcoSURF surface)
{
    gcoSURF srcSurface;
    ...
    if(image->magic != KHR_EGL_IMAGE_MAGIC_NUM)
    {
        gcmPRINT("image is wrong!");
        return;
    }
    ...
    srcSurface = image->srcSurface;

    if(srcSurface != gcvNULL)
    {
        gcoSURF_Destroy(srcSurface);
    }
...
static EGLBoolean glshSwapbuffer(EGLDisplay Display, EGLSurface Surface, gcfEGL_DO_SWAPBUFFER Callback)
{
    EGLBoolean  done = EGL_FALSE;

    gcmHEADER_ARG("Display=%p Surface=%p Callback=%p", Display, Surface, Callback);
    ...
    do
    {
        ...
        if (context != gcvNULL)
        {
            glmBATCH(context,
                     ...
                     (glshBatchSwapbuffer(context, Display, Surface, Callback)));
        }
    }
    while (gcvFALSE);
#endif

    gcmFOOTER_ARG("%d", done);
    return done;
}
