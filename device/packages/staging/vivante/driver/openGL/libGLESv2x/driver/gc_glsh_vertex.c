...
/* Construct the gcoVERTEXARRAY object. */
    ...
    for (i = 0; i < gcmCOUNTOF(Context->vertexArrayGL); ++i)
    {
        ...
        Context->vertexArray[i].genericSize     = 4;
    }
#else
...
GLenum glshVertexAttribArray(IN GLContext Context,
                             IN GLuint Index,
                             IN GLboolean Enable)
{
#if gldUSE_VERTEX_ARRAY
    ...
    vertexArray[Index].enable = Enable;

    /* Disable batch optmization. */
    Context->batchDirty = gcvTRUE;
    if (Enable)
    {
    ...
    )
{
	...
	glmENTER1(glmARGUINT, index)
	{
        ...
        glmBATCH(context,
                 break,
                 break,
                 (glshBatchVertexAttributeSet(context, index, gcvTRUE)));

#endif

        /* Call the function. */
        glshVertexAttribArray(context, index, GL_TRUE);
	}
	glmLEAVE()
#endif
}
    ...
    )
{
    ...
    glmENTER1(glmARGUINT, index)
	{
        ...
        glmBATCH(context,
                 break,
                 break,
                 (glshBatchVertexAttributeSet(context, index, gcvFALSE)));
#endif

        /* Call the function. */
        glshVertexAttribArray(context, index, GL_FALSE);
	}
	glmLEAVE()
#endif
}
                      ...
                      )
{
		      ...
		      glmARGINT, normalized, glmARGINT, stride, glmARGPTR, pointer)
	{
        ...
        glmBATCH(context,
                 break,
                 break,
                 (glshBatchVertexAttributePointer(context, index, size, type, normalized, stride, pointer, context->arrayBuffer)));
#endif

        /* Call the funciotn. */
        glshVertexAttribPointer(context, index, size, type, normalized, stride, pointer, context->arrayBuffer);
	}
	glmLEAVE()
#endif
}

GLenum glshVertexAttribPointer(IN GLContext Context,
                               IN GLuint Index,
                               IN GLint Size,
                               IN GLenum Type,
                               ...
                               IN GLBuffer Buffer)
{
    ...
    GLAttribute *       vertexArray;
        ...
        {
            if ((Buffer == gcvNULL) && (Pointer != gcvNULL))
            {
        ...
        switch (Type)
        {
            ...
            case GL_INT_10_10_10_2_OES:
                ...
                format     = (Type == GL_UNSIGNED_INT_10_10_10_2_OES) ? gcvVERTEX_UNSIGNED_INT_10_10_10_2 : gcvVERTEX_INT_10_10_10_2;
        ...
        vertexArrayGL[Index].buffer   = Buffer;
        ...
        vertexArray[Index].normalized = Normalized;
        vertexArray[Index].stride     = autoStride;
        vertexArray[Index].buffer     = Buffer;
        ...
        {
            gcmVERIFY_OK(gcoSTREAM_SetStride(Buffer->stream, autoStride));
    ...
    )
{
    ...
    glmENTER()
	{
        ...
        *Context = context;
        ...
        glmBATCH(context,
                 break,
                 { result = gcvTRUE; break; } glmBATCH_NOP,
                 (glshBatchVertexAttribute(context, Index, V0, V1, V2, V3)));
#endif

        /* Do the call. */
        result = glshVertexAttrib(context, Index, V0, V1, V2, V3) == GL_NO_ERROR;
    }
	glmLEAVE()
...
GLenum glshVertexAttrib(IN GLContext Context,
                        IN GLuint Index,
                        IN GLfloat X,
                        IN GLfloat Y,
                        IN GLfloat Z,
...
#if gldUSE_VERTEX_ARRAY
    Context->vertexArray[Index].genericValue[0] = X;
    Context->vertexArray[Index].genericValue[1] = Y;
    Context->vertexArray[Index].genericValue[2] = Z;
    Context->vertexArray[Index].genericValue[3] = W;

    /* Tmp solution for VAO generic value */
    if (Context->vertexArrayObject != gcvNULL)
    {
        Context->vertexArrayObject->vertexArray[Index].genericValue[0] = X;
        Context->vertexArrayObject->vertexArray[Index].genericValue[1] = Y;
        Context->vertexArrayObject->vertexArray[Index].genericValue[2] = Z;
        Context->vertexArrayObject->vertexArray[Index].genericValue[3] = W;
    }
#else
    Context->genericVertex[Index][0]    = X;
    Context->genericVertex[Index][1]    = Y;
    Context->genericVertex[Index][2]    = Z;
    Context->genericVertex[Index][3]    = W;
    ...
    )
{
    ...
    vertexArray[index].genericSize       = 1;
    ...
    )
{
    ...
    vertexArray[index].genericSize       = 2;
    ...
    )
{
    ...
    vertexArray[index].genericSize       = 3;
    ...
    )
{
    ...
    vertexArray[index].genericSize       = 4;
