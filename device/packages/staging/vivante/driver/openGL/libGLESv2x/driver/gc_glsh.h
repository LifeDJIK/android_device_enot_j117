...
#define gldUSE_VERTEX_ARRAY             1
#define gldBLOB_CACHE_KEY_SIZE          32
#define gldBLOB_CACHE_MD5_SIZE          16
#define gldBLOB_CACHE_MAX_BINARY_SIZE   4 << 10
#define gldNAMED_OBJECTS_HASH           32
#define gldVERTEX_ELEMENT_COUNT         16
#define gldTEXTURE_SAMPLER_COUNT        32
#define gldPATCHES                      1
#define gldFBO_DATABASE                 0
#define gldBATCHING                     0
...
#define gldZONE_PATCH               (gcvZONE_API_ES20 | (1 << 10))
...
#define glmLEAVE2(Type1, Value1, Type2, Value2) \
    } \
    while (GL_FALSE); \
	glmGetApiEndTime(context); \
	if (_HasError) { \
        ...
        gcmFOOTER_ARG(#Value1 "=" Type1 \
                  " " #Value2 "=" Type2, \
				  Value1, Value2; \
	} \

#define glmLEAVE3(Type1, Value1, Type2, Value2, Type3, Value3) \
    } \
    while (GL_FALSE); \
	glmGetApiEndTime(context); \
	if (_HasError) { \
...
#define glmLEAVE4(Type1, Value1, Type2, Value2, Type3, Value3, Type4, Value4) \
    } \
    while (GL_FALSE); \
	glmGetApiEndTime(context); \
	if (_HasError) { \
...
typedef union gctConstantValue
{
    gctBOOL        b;
    gctUINT8       u8;
    gctUINT16      u16;
...
struct _GLProgram
{
    ...
    sampleMap[gldTEXTURE_SAMPLER_COUNT];

#if gldFBO_DATABASE
    /* Mapped texture objects. */
    GLTexture           textures[gldTEXTURE_SAMPLER_COUNT];
    GLTexture           savedTextures[gldTEXTURE_SAMPLER_COUNT];
...
#if gldPATCHES
    glsBATCH_STATE_UNIFORM *    batchHead;
    glsBATCH_STATE_UNIFORM *    batchTail;
#endif
};
...
#if gldPATCHES
    struct glsBATCH_STATE_TEXTURE   batchedState;
#endif
};
...
#if gldFBO_DATABASE
    glsDATABASE_CONTAINER   database;
#endif
};
...
struct _GLContext
{
    ...
    GLTexture           texture2D[gldTEXTURE_SAMPLER_COUNT];
    struct _GLTexture   defaultCube;
    GLTexture           textureCube[gldTEXTURE_SAMPLER_COUNT];
    struct _GLTexture   default3D;
    GLTexture           texture3D[gldTEXTURE_SAMPLER_COUNT];
    struct _GLTexture   defaultExternal;
    GLTexture           textureExternal[gldTEXTURE_SAMPLER_COUNT];
    GLProgram           program;
    GLboolean           programDirty;
    GLenum              mipmapHint;
    GLenum              fragShaderDerivativeHint;
    gcePRIMITIVE        lastPrimitiveType;
    GLTexture           bounded[gldTEXTURE_SAMPLER_COUNT];
...
#if gldPATCHES
    glsPATCH_INFO       patchInfo;
    glsBATCH_INFO           batchInfo;
#endif
};
...
GLenum glshClear(IN GLContext Context,
                 IN GLbitfield Mask);

GLenum glshDrawElements(IN GLContext Context,
                        IN GLenum Mode,
                        IN GLsizei Count,
                        IN GLenum Type,
...
GLenum glshDrawArrays(IN GLContext Context,
                      IN GLenum Mode,
                      IN GLint First,
                      IN GLsizei Count);
...
GLenum glshBindFramebuffer(IN GLContext Context,
                           IN GLenum Target,
...
GLenum glshFrontFace(IN GLContext Contetx,
                     IN GLenum FrontFace);
...
GLTexture glshNewTexture(IN GLContext Context,
                         IN GLuint Name);
