...
#define gldSHOW_SHADER_SOURCE   0
    ...
    )
{
    ...
    for (i = 0; i < Count; ++i)
    {
        ...
        if (uniform->varCategory != gcSHADER_VAR_CATEGORY_NORMAL)
            continue;
    ...
    for (i = 0; i < Count; ++i)
    {
        ...
        else
        {
            ...
            if (gcmIS_SUCCESS(gcoOS_StrCmp(name, "uniBonesM"))
                ...
                ||
                gcmIS_SUCCESS(gcoOS_StrCmp(name, "uniModelViewProjectionMatrix"))
                )
            {
                if (++count == 3)
                {
...
#if gldPATCHES
            /* Get type and length of uniform once. */
            gcmVERIFY_OK(gcUNIFORM_GetType(uniform, &slot->type, &slot->length));

            /* Determine the size for one element (for arrays). */
            switch (slot->type)
            {
                case gcSHADER_FLOAT_X1:
                case gcSHADER_BOOLEAN_X1:
                case gcSHADER_INTEGER_X1:
                default:
                    slot->elementLength = 1 * 4;
                    break;

                case gcSHADER_FLOAT_X2:
                case gcSHADER_BOOLEAN_X2:
                case gcSHADER_INTEGER_X2:
                    slot->elementLength = 2 * 4;
                    break;

                case gcSHADER_FLOAT_X3:
                case gcSHADER_BOOLEAN_X3:
                case gcSHADER_INTEGER_X3:
                    slot->elementLength = 3 * 4;
                    break;
                ...
                case gcSHADER_BOOLEAN_X4:
                case gcSHADER_INTEGER_X4:
                    slot->elementLength = 4 * 4;
                    break;

                case gcSHADER_FLOAT_3X3:
                    slot->elementLength = 9 * 4;
                    break;

                case gcSHADER_FLOAT_4X4:
                    slot->elementLength = 16 * 4;
                    break;
                ...
                case gcSHADER_SAMPLER_3D:
                    slot->elementLength = 1 * 4;
                    gcUNIFORM_GetSampler(uniform, &slot->sampler);
                    break;
            }
#endif
        }
...
#if gcdALPHA_KILL_IN_SHADER && gldPATCHES
    if (glshQueryPatchAlphaKill(Context))
    {
        flags |= gcvSHADER_USE_ALPHA_KILL;
    }
#endif

    status = gcLinkShaders(vertexShader, fragmentShader, flags,
                           &Program->statesSize,
                           &Program->states,
                           &Program->hints);
       ...
       IN OUT LTCValue *             SourceValue)
{
    ...
    case gcSL_UNIFORM:
    {
        ...
        */
        ...
        if (combinedOffset >= uniform->arraySize * shader_type_info[uniform->u.type].rows )
        {
        ...
        data = (gctFLOAT *)glUniform->data +
                  combinedOffset * shader_type_info[uniform->u.type].components;
...
static gctPOINTER   _CachedResultArray = gcvNULL;
static gctSIZE_T    _CurResultArraySize = 0;
    ...
    )
{
    ...
    if (_CurResultArraySize >= Size)
    {
        /* cached result array is larger than requested */
        gcmASSERT(_CachedResultArray != gcvNULL);
        *Memory = _CachedResultArray;
        return gcvSTATUS_OK;
    }

    if (_CachedResultArray != gcvNULL)
    {
        /* requested size is larger than cached size, reallocate */
        gcoOS_Free(Os, _CachedResultArray);
        _CachedResultArray = gcvNULL;
        _CurResultArraySize = 0;
    }

    /* allocate a result array at least for 100 instructions */
    gcmASSERT(_CurResultArraySize == 0);
    _CurResultArraySize = Size > sizeof(LTCValue)*100 ? Size
                                                      : sizeof(LTCValue)*100;
    status = gcoOS_Allocate(Os,
                            _CurResultArraySize,
                            (gctPOINTER *) &_CachedResultArray);
    *Memory = _CachedResultArray;
    ...
    )
{
    ...
    gcmONERROR(_getResultArray(gcvNULL,
