...
{
    GLObjectType objectType = GLObject_Unknown;
    GLShader shader;
    gctPOINTER pointer = gcvNULL;
	GLuint result = 0;
	GLboolean goToEnd = gcvFALSE;

	glmENTER1(glmARGENUM, type)
	{
	...
	result = shader->object.name;
	}
	glmLEAVE1(glmARGUINT, result);
	return result;
}
    ...
    )
{
	...
	glmENTER4(glmARGUINT, shader, glmARGINT, count, glmARGPTR, string, glmARGPTR, length)
	{
#if gcdDUMP_API
...
{
    gctMD5_Byte blobCacheKey[gldBLOB_CACHE_KEY_SIZE];
    ...
    )
{
	...
	glmENTER2(glmARGUINT, shader, glmARGENUM, pname)
	{
    ...
    )
{
	...
	glmENTER2(glmARGUINT, shader, glmARGINT, bufsize)
	{
	...
	glmLEAVE1(glmARGINT, gcmOPT_VALUE(length));
	return;
OnError:
	gl2mERROR(GL_INVALID_OPERATION);
    ...
    )
{
    ...
    glshPatchDeleteProgram(context, object);
#endif
...
#if gldPATCHES
    /* Send program to patch logic. */
    glshPatchLink(context, programObject);
#endif
    ...
    )
{
	...
	glmENTER2(glmARGUINT, program, glmARGENUM, pname)
	{
    ...
    )
{
    ...
    glmENTER3(glmARGUINT, program, glmARGUINT, index, glmARGINT, bufsize)
	{
	...
	glmLEAVE4(glmARGINT, gcmOPT_VALUE(length), glmARGINT, gcmOPT_VALUE(size), glmARGENUM, gcmOPT_VALUE(type), glmARGPTR, name);
	return;
OnError:
    gl2mERROR(GL_INVALID_OPERATION);
	gcmFOOTER_ARG("error=0x%04x", GL_INVALID_OPERATION);
    ...
    )
{
	...
	glmENTER2(glmARGUINT, program, glmARGPTR, name)
	{
    ...
    )
{
	...
	glmENTER1(glmARGUINT, program)
	{
        ...
        glmBATCH(context,
                 break,
                 break,
                 (glshBatchProgram(context, program)));
#endif
    ...
    )
{
	...
	glmENTER2(glmARGUINT, program, glmARGINT, maxcount)
	{
	...
	glmLEAVE1(glmARGINT, gcmOPT_VALUE(count));
}
    ...
    )
{
	...
	glmENTER2(glmARGUINT, program, glmARGINT, bufsize)
	{
    ...
    )
{
	...
	glmENTER2(glmARGUINT, program, glmARGUINT, shader)
	{
    ...
    )
{
	...
	glmENTER3(glmARGUINT, program, glmARGUINT, index, glmARGPTR, name)
	{
	gcmTRACE(gcvLEVEL_VERBOSE, "name=%s", name);
    ...
    )
{
    ...
    glmBATCH(Context,
             return,
             return,
             (glshBatchUniform(Context, Location, Type, Count, Values)));
#endif
    ...
    error = glshSetUniformData(program,
                               uniform,
                               Type,
                               Count,
                               index,
...
{
#if gcdNULL_DRIVER < 3
    glmENTER2(glmARGINT, location, glmARGFLOAT, x)
	{
...
{
#if gcdNULL_DRIVER < 3
	glmENTER3(glmARGINT, location, glmARGINT, count, glmARGPTR, v)
	{
...
{
#if gcdNULL_DRIVER < 3
    glmENTER3(glmARGINT, location, glmARGFLOAT, x, glmARGFLOAT, y)
	{
...
{
#if gcdNULL_DRIVER < 3
	glmENTER4(glmARGINT, location, glmARGFLOAT, x, glmARGFLOAT, y, glmARGFLOAT, z)
	{
...
{
#if gcdNULL_DRIVER < 3
    glmENTER5(glmARGINT, location, glmARGFLOAT, x, glmARGFLOAT, y,
		glmARGFLOAT, z, glmARGFLOAT, w)
	{
...
{
#if gcdNULL_DRIVER < 3
    glmENTER4(glmARGINT, location, glmARGINT, count, glmARGINT, transpose, glmARGPTR, value)
	{
...
{
#if gcdNULL_DRIVER < 3
    glmENTER2(glmARGINT, location, glmARGINT, x)
	{
...
{
#if gcdNULL_DRIVER < 3
    glmENTER3(glmARGINT, location, glmARGINT, x, glmARGINT, y)
	{
...
{
#if gcdNULL_DRIVER < 3
    glmENTER4(glmARGINT, location, glmARGINT, x, glmARGINT, y, glmARGINT, z)
	{
...
{
#if gcdNULL_DRIVER < 3
    glmENTER5(glmARGINT, location, glmARGINT, x, glmARGINT, y, glmARGINT, z, glmARGINT, w)
	{
    ...
    )
{
	...
	glmENTER3(glmARGUINT, program, glmARGINT, location, glmARGPTR, params)
	{
    ...
    )
{
    ...
    glmENTER2(glmARGUINT, program, glmARGINT, location)
	{
    ...
    )
{
	...
	glmENTER5(glmARGINT, n, glmARGPTR, shaders, glmARGENUM, binaryformat,
		      glmARGPTR, binary, glmARGINT, length)
	{
    ...
    )
{
	...
	glmENTER5(glmARGUINT, program, glmARGINT, bufSize, glmARGPTR, length,
		      glmARGPTR, binaryFormat, glmARGPTR, binary)
	{
