    ...
    )
{
    ...
    framebuffer->color.texture = 0;
    framebuffer->color.renderMipmap = gcvFALSE;
    ...
    framebuffer->depth.texture = 0;
    framebuffer->depth.renderMipmap = gcvFALSE;
    framebuffer->stencil.renderMipmap = gcvFALSE;
    ...
    )
{
    ...
    glshRemoveDatabase(&Framebuffer->database);
#endif
    ...
    )
{
	...
	glmENTER2(glmARGENUM, Target, glmARGUINT, Framebuffer)
	{
        ...
        glmBATCH(context,
                 break,
                 break,
                 (glshBatchFramebuffer(context, Target, Framebuffer)));
#endif

        /* Do the binding. */
        glshBindFramebuffer(context, Target, Framebuffer);
    }
    glmLEAVE()
#endif
}
    ...
    if (Context->framebuffer != object)
    {
        ...
        if (Context->framebuffer != gcvNULL)
        {
            ...
            )
            {
                ...
                {
                    texture = (GLTexture) Context->framebuffer->color.object;
                    ...
                    if (texture->target == GL_TEXTURE_2D)
                    {
                        ...
                        gcmONERROR(gcoTEXTURE_GetMipMap(texture->texture,
                                                        ...
                                                        &surface));
                        if ((Context->framebuffer->color.renderMipmap )
                            &&
                            (surface != Context->framebuffer->color.surface)
                            )
                        {
                            gcmONERROR(gcoTEXTURE_RenderIntoMipMap(texture->texture,
                                                                   Context->framebuffer->color.level));
        ...
        if(object != gcvNULL
           ...
           && object->color.object->type == GLObject_Texture)
        {
            texture = (GLTexture)object->color.object;
            ...
            if (texture->target == GL_TEXTURE_2D)
            {
                ...
                if(object->color.renderMipmap
                   && surface != object->color.surface)
                {
                    gcmONERROR(gcoTEXTURE_RenderIntoMipMap(texture->texture,
                                        object->color.level));
    ...
    )
{
    ...
    /*GLuint  oldTex = 0;
    gcoSURF oldSurface = gcvNULL;*/
    gctBOOL renderIntoMipmap = gcvFALSE;
    ...
    /* Find texture object. */
    if ((Texture == 0) && (!context->copyTexShaderInside))
    {
    ...
    else
    {
        ...
        if (object == gcvNULL)
        {
            if (context->copyTexShaderInside)
            {
                object = &context->default2D;
                Texture = (GLuint)~0;
            }
            else
            {
    ...
    case GL_COLOR_ATTACHMENT0:
        ...
        /*oldTex     = context->framebuffer->color.texture;
        oldSurface = context->framebuffer->color.surface;*/
        ...
        context->framebuffer->color.renderMipmap   = renderIntoMipmap;
        ...
        */
        ...
        context->framebuffer->color.texture = Texture;
    ...
    case GL_DEPTH_ATTACHMENT:
...
#endif
        /*oldTex     = context->framebuffer->depth.texture;
        oldSurface = context->framebuffer->depth.surface;*/
        ...
        context->framebuffer->depth.renderMipmap   = renderIntoMipmap;

        context->framebuffer->depth.target  = target;

        context->framebuffer->depth.texture = Texture;
    ...
    case GL_STENCIL_ATTACHMENT:
        ...
        /*oldTex     = context->framebuffer->stencil.texture;
        oldSurface = context->framebuffer->stencil.surface;*/
        ...
        context->framebuffer->stencil.renderMipmap   = renderIntoMipmap;

        context->framebuffer->stencil.target  = target;

        /*oldTex = context->framebuffer->stencil.texture;*/

        context->framebuffer->stencil.texture = Texture;
    ...
    if(object != gcvNULL && object->image != gcvNULL && surface != gcvNULL)
       _glshSetImageSrc(object->image,surface);
