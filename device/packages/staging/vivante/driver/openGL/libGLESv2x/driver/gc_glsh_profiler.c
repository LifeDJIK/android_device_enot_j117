	...
	)
{
	...
	/* TODO: start frame is not supported yet, because some information has to be saved in the first frame
	gcoOS_GetEnv(Context->os, "VPROFILER_BEGIN_FRAME", &env);
    if ((env == gcvNULL) || (env[0] ==0))
    {
	...
	gcoOS_GetEnv(Context->os, "VPROFILER_END_FRAME", &env);
    if ((env == gcvNULL) || (env[0] ==0))
    {
	...
	Context->profiler.frameStartNumber = 0;
    Context->profiler.frameEndNumber = 0;
	gcoOS_GetEnv(Context->os, "VPROFILER_FRAME_NUM", &env);
    if ((env != gcvNULL) && (env[0] !=0))
    {
		int frameNum;
		gcoOS_StrToInt(env, &frameNum);
		if (frameNum > 1)
			Context->profiler.frameEndNumber = (gctUINT32)frameNum - 1;
	}
	...
	)
{
	...
	if (Context->profiler.frameEndNumber > 0 && Context->profiler.frameNumber > Context->profiler.frameEndNumber)
	{
		Context->profiler.frameNumber++;
		return;
	}
	...
	if (Context->profiler.enable)
	{
        ...
        if (Context->profiler.drvEnable)
        {
...
#if gcdNEW_PROFILER_FILE
            gcmWRITE_CONST(VPG_ES20_TIME);
			for (i = 0; i < GLES2_NUM_API_CALLS; ++i)
			{
        		if (Context->profiler.apiCalls[i] > 0)
        		{
					gcmWRITE_COUNTER(VPG_ES20_TIME + 1 + i, (gctINT32) Context->profiler.apiTimes[i]);
				}
			}
			gcmWRITE_CONST(VPG_END);
#endif
			...
			for (i = 0; i < GLES2_NUM_API_CALLS; ++i)
			{
				...
				/*if (Context->profiler.apiCalls[i] > 0)
				{
					gcoOS_Print("framenumber: %d, api %d, count: %d, time: %ld\n",
						Context->profiler.frameNumber, i, Context->profiler.apiCalls[i], Context->profiler.apiTimes[i]);
				}*/
