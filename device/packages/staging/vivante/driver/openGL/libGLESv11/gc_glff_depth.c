    ...
    if (Context->depthStates.polygonOffsetDirty)
    {
            ...
            != gcvSTATUS_TRUE)
        {
            /* Use alternate path to program depth bias.
               We assume slope to be 1. So depthfactor should also be multiplied
               by glvFRACONEOVER65535
            */
            Context->hashKey.hashDepthBiasFixEnabled = depthBiasEnabled;
            Context->hashKey.hashDepthBias = glmFRAC2FLOAT(depthBias)
                                           + glmFRAC2FLOAT(
                                                glmFRACMULTIPLY (depthScale,glvFRACONEOVER65535)
                                             );
        ...
        /* Reset dirty flag */
        Context->depthStates.polygonOffsetDirty = gcvFALSE;
    }
