	...
	)
{
	...
	for (i = 0; i < Context->texture.pixelSamplers; i++)
	{
		...
		/* Skip the stage if disabled. */
		if (Context->texture.sampler[i].stageEnabled == gcvFALSE)
		{
			continue;
		}

		/* Get a shortcut to the current sampler's bound texture. */
		texture = Context->texture.sampler[i].binding;

		/* Skip no scaled texture. */
		if (texture->uscale == 1.0f || texture->uscale == 0.0f)
		{
			continue;
		}

		/* Found texture matrix. */
		matrix = Context->matrixStackArray[glvTEXTURE_MATRIX_0 + i].topMatrix;
		...
		if (matrix->type == glvFLOAT)
		{
			...
			if (Apply)
			{
				glmMATFLOAT(matrix, 0, 0) *= texture->uscale;
			}
			else
			{
				glmMATFLOAT(matrix, 0, 0) /= texture->uscale;
			...
			e = glmMATFLOAT(matrix, 0, 0) - glvFRACONE;

			if (e > -glvFRACONE / 0x2000 && e < glvFRACONE / 0x2000)
			{
				glmMATFLOAT(matrix, 0, 0) = glvFRACONE;
			}
		}
		else
		{
			GLfixed factor = glmFIXED2FLOAT(texture->uscale);
			GLfloat e;

			if (Apply)
			{
				glmMATFIXED(matrix, 0, 0) =
					glmFIXEDMULTIPLY(glmMATFIXED(matrix, 0, 0), factor);
			}
			else
			{
				glmMATFIXED(matrix, 0, 0) =
					glmFIXEDDIVIDE(glmMATFIXED(matrix, 0, 0), factor);
			}

			/* Precision correction. */
			e = glmMATFIXED(matrix, 0, 0) - glvFRACONE;

			if (e > -glvFRACONE / 0x2000 && e < glvFRACONE / 0x2000)
			{
				glmMATFIXED(matrix, 0, 0) = glvFRACONE;
			}
		}
    ...
    )
{
    ...
    glmENTER3(glmARGENUM, Mode, glmARGINT, First, glmARGINT, Count)
    {
        ...
        do
        {
            ...
            gcmERR_BREAK(_ApplyTextureScale(context, gcvTRUE));
            ...
            gcmERR_BREAK(_ApplyTextureScale(context, gcvFALSE));
