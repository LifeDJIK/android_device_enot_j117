    ...
    )
{
    ...
    do
    {
        ...
        /* Reset uscale value to not scaled. */
        Texture->uscale     = 1.0f;
    ...
    )
{
    ...
    do
    {
        ...
        /* Zero buffers. */
        gcmVERIFY_OK(gcoOS_ZeroMemory(&Context->texture.textureList,
                                      sizeof(Context->texture.textureList)));
    ...
    for (i = 0; i < Context->texture.pixelSamplers; i++)
    {
        ...
        )
        {
            ...
            if (texture->object == gcvNULL)
            {
                ...
                /* Resolve needs 16x4 pxiel alignment, while texture needs 4x4
                 * alignment on some hardwares. This is to align texture to
                 * 16x4 to meet resolve limitation.
                 * TODO: Check horizontal alignement feature first. */
                if (gcmALIGN(width, 4) & 0x0F)
                {
                    scaledWidth = gcmALIGN(width, 16);
                    texture->uscale = (gctFLOAT) width / scaledWidth;
                }
                else
                {
                    scaledWidth = width;
    ...
    )
{
              ...
              glmARGENUM, Target)
    {
        ...
        */
        gcmERR_BREAK(gcoTEXTURE_AddMipMap(texture->object, 0, Format, gcvSURF_A8R8G8B8, width, height, 0, 0, gcvPOOL_DEFAULT, &Surface));
    ...
    glmENTER2(glmARGENUM, Target, glmARGPTR, Image)
    {
        ...
        )
        {
            ...
            else
            {
                ...
                struct gc_private_handle_t * handle =
                   (struct gc_private_handle_t *) attributes.privHandle;


                if (attributes.width != (gctUINT) handle->width)
                {
                    /* Texture surface width is not equel to buffer width, so
                     * texture surface is scaled to 16 pixel boundary in
                     * gralloc. This means its original size (buffer size) is
                     * not 16 pixel aligned. Here we get its buffer size from
                     * buffer handle and compute uscale factor to scaling back
                     * in later composition. */
                    texture->uscale   = (gctFLOAT) handle->width / attributes.width;

                    attributes.width  = (gctUINT) handle->width;
                    attributes.height = (gctUINT) handle->height;
                }
            }
