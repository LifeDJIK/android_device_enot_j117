    ...
    )
{
    ...
    case EGL_OPENGL_ES_API:
        context = thread->esContext;
        break;
    case EGL_OPENVG_API:
        context = thread->vgContext;
        break;
    case EGL_OPENGL_API:
        context = thread->glContext;
        break;
    }
    ...
    )
{
    ...
    EGLBoolean cleanPath = EGL_FALSE;
#endif
...
#if __VIV_GLES3_HALTI__
        else if (thread->context->client == 3)
        {
            cleanPath = EGL_TRUE;
        }
#endif
    ...
    /* Should go into each API to only return proc address for enabled extensions
    ** currently only enabled for es30
    */
#if __VIV_GLES3_HALTI__
    if (cleanPath)
    {
    ...
    {
        gcmVERIFY_OK(gcoOS_StrLen(appendix, &appendixLen));
