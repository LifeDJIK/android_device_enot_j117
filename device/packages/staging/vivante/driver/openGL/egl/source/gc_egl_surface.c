...
#if _IS_ANDROID_
static gctBOOL _isNotAndroidBenchmark()
{
	gctUINT i = 0;
    gctSTRING BenchList[] = {
		"com.rightware.tdmm2",
		"com.vivantecorp.graphics.mm",
		"com.aurorasoftworks.quadrant",
		"com.antutu",
		"com.smartbench",
		"com.threed.jpct.bench",
		"se.nena.nenamark",
		"com.qualcomm.qx.neocore",
		"com.glbenchmark",
		"com.android.benchmark",
    };

    for(i = 0; i < gcmCOUNTOF(BenchList); i++ )
    {
		if(gcoOS_DetectProcessByName(BenchList[i]))
        {
			i--;
	        break;
	    }
	...
	return (i == gcmCOUNTOF(BenchList))? gcvTRUE : gcvFALSE;
}
#endif
    ...
    )
{
    ...
    do
    {
        ...
        gcmASSERT(Surface->fbWrappers[2]    == gcvNULL);
#ifdef __QNXNTO__
        ...
        if ((Surface->fbDirect) /*&& (Surface->fbInfo.isCompositor)*/)
        {
#if !defined(ANDROID)
...
/* 355_FB_MULTI_BUFFER */
            ...
            if (Surface->openVG)
            {
                ...
                {       /* Create the first backbuffer. */
                    ...
                    Surface->fbInfo.logicals[1] =
                        (gctPOINTER)((gctUINT32)Surface->fbInfo.logicals[0] + backOffset);
                    Surface->fbInfo.physicals[1] = Surface->fbInfo.physicals[0] + backOffset;
                ...
                {       /* Setup the 2nd backbuffer. */
                    gctUINT32 backOffset = (gctUINT32)(Surface->fbInfo.stride * 2 *
                                            Surface->fbInfo.height / Surface->fbInfo.multiBuffer);
                    gcmASSERT(Surface->fbWrappers[2] == gcvNULL);
                    gcmERR_BREAK(gcoSURF_ConstructWrapper(gcvNULL, &Surface->fbWrappers[2]));
                    /* Calculate the memory address for backbuffer. */
                    Surface->fbInfo.logicals[2] =
                        (gctPOINTER)((gctUINT32)Surface->fbInfo.logicals[0] + backOffset);
                    Surface->fbInfo.physicals[2] = Surface->fbInfo.physicals[0] + backOffset;

                    /* Set the underlying frame buffer surface. */
                    gcmERR_BREAK(gcoSURF_SetBuffer(
                        Surface->fbWrappers[2],
                        gcvSURF_BITMAP,
                        ResolveFormat,
                        Surface->fbInfo.stride,
                        Surface->fbInfo.logicals[2],
                        Surface->fbInfo.physicals[2]
                        ));
        ...
        if (Surface->openVG)
        {
...
#ifndef __QNXNTO__
            /* Force go internal render target path for 355 . */
            if (0)
#else
...
#endif
            {
                ...
                {       /* For FB_MULTI_BUFFER, this is the backbuffer where it should render onto. */
                    Surface->renderTarget = Surface->fbWrappers[1];
                    ...
                    gcmERR_BREAK(gcoSURF_SetColorType(
                        Surface->fbWrappers[0], Surface->colorType
                        ));

                    if (Surface->fbWrappers[2] != gcvNULL)
                    {
                        gcmERR_BREAK(gcoSURF_SetOrientation(
                            Surface->fbWrappers[2],
                            gcvORIENTATION_TOP_BOTTOM
                        ...
                        /* Set render surface type. */
                        gcmERR_BREAK(gcoSURF_SetColorType(
                            Surface->fbWrappers[2], Surface->colorType
                            ));
                    }
        ...
        else
        {
            ...
            else if (Surface->fbWrappers[0] != gcvNULL)
            {
                /* Get the aligned size of the surface and the stride. */
                gcmERR_BREAK(gcoSURF_GetAlignedSize(
                    Surface->fbWrappers[0],
        ...
        Surface->drawable.depthHandle   = (void*)Surface->depthBuffer;
        Surface->drawable.stencilHanlde = gcvNULL;
#endif
    ...
    do
    {
        ...
        {
            /* Destroy the back buffer wrapper surface. */
            gcmERR_BREAK(gcoSURF_Destroy(Surface->fbWrappers[2]));
            Surface->fbWrappers[2] = gcvNULL;
        }
#endif

#if __VIV_GLES3_HALTI__
        gcmVERIFY_OK(gcoOS_ZeroMemory(&Surface->drawable, sizeof(EGLDrawable)));
#endif
    }
    while (gcvFALSE);
...
#if __VIV_GLES3_HALTI__
        /* TODO: refine later */
        result = _SetDrawable(thread, thread->context, &Surface->drawable,
                              &thread->context->read->drawable);
#else
    ...
    )
{
...
#ifndef __QNXNTO__
    /* 355_FB_MULTI_BUFFER */
    surface->fbWrappers[0] = surface->fbWrappers[1] = surface->fbWrappers[2] = gcvNULL;
#endif
	...
	if (Config != gcvNULL)
    {
        ...
        if ((Thread->api != EGL_OPENVG_API) && (Config->bufferSize < 32) &&
            (gcoHAL_IsFeatureAvailable(gcvNULL, gcvFEATURE_PE_DITHER_FIX) != gcvTRUE)
#if _IS_ANDROID_
			&& _isNotAndroidBenchmark()
#endif
			)
        {
    ...
    do
    {
        ...
        */
        ...
        if (surfaceType == EGL_WINDOW_BIT)
        {
            ...
            Surface->fbInfo.physicals[0] -= baseAddress;
#ifdef __QNXNTO__
