...
#   if !gcdGPU_LINEAR_BUFFER_ENABLED
                        ...
                        (size.y == draw->bitsHeight))
                    {
                        ...
                        {
                            /* Output texture is aligned to 16 in gralloc. */
                            size.x = gcmALIGN(size.x, 16);
                        }
                    }
#   endif
            ...
            else
            {
                ...
                pSurface = (s_nSurfaceIndex++ % 2) ? draw->fbWrapper2 : draw->fbWrappers[0];
            ...
            else
            {
                ...
                if (flip)
                {
                    ...
                    if (draw->renderListEnable)
                    {
                        ...
                        {
                            rendBuf = backBuffer.origin.y * draw->fbInfo.multiBuffer / draw->fbInfo.height;
                            rendBuf %= draw->fbInfo.multiBuffer;
                        }

                        dispatch = _GetDispatch(Thread, gcvNULL);
                        if ((dispatch != gcvNULL)
                        &&  (dispatch->resolveVG != gcvNULL)
                        )
                        {
                            if (draw->fbWrappers[rendBuf] != gcvNULL)
                            {
                                (*dispatch->resolveVG)(Thread->context->context, draw->fbWrappers[rendBuf]);
                            }
                        }
                    }
                    ...
                    if (!draw->renderListEnable)
                    {
                        if (draw->fbInfo.multiBuffer > 1)
                        {
                            ...
                            /* Set Render Buffer. */
						    draw->renderTarget = draw->fbWrappers[rendBuf];
                            _SetBuffer(thread, draw->renderTarget);
                        }
                    }
                }
                else
                {
                    ...
                    if (draw->renderListEnable)
                    {
                        ...
                        {
                            if (draw->fbWrappers[0] != gcvNULL)
                            {
                                (*dispatch->resolveVG)(Thread->context->context, draw->fbWrappers[0]);
                                gcoHAL_Commit(gcvNULL, gcvTRUE);
                            }
                        }
                    }
...
static void eglfDoSwapBuffer(EGLDisplay Dpy, EGLSurface Draw)
{
    ...
    )
{
    ...
    {
        /* Get dispatch table. */
        veglDISPATCH * dispatch = _GetDispatch(veglGetThreadData(), gcvNULL);

        /* Check if there is a swapbuffer function. */
        if ((dispatch != gcvNULL) && (dispatch->swapBuffer != gcvNULL))
        {
            /* Call the swapbuffer function. */
            if ((*dispatch->swapBuffer)(Dpy, Draw, eglfDoSwapBuffer))
            {
                /* The driver has handled the swap buffers. */
    ...
    do
    {
        ...
        if ((bits != gcvNULL) && ((gcmPTR2INT(bits) & (alignment - 1)) == 0))
        {
            ...
            status = gcoSURF_MapUserSurface(
                tempSurface, 0, bits, gcvINVALID_ADDRESS
                );
    ...
    )
{
    ...
    gcmHEADER_ARG("Dpy=0x%x Draw=0x%x", Dpy, Draw);
    gcmDUMP_API("${EGL eglGetRenderBufferANDROID 0x%08X 0x%08X}", Dpy, Draw);
