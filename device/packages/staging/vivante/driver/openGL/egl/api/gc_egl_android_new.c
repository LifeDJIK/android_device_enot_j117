    ...
    )
{
    ...
    ret = Window->query(Window, NATIVE_WINDOW_WIDTH, &width);
    ret |= Window->query(Window, NATIVE_WINDOW_HEIGHT, &height);
    ret |= Window->query(Window, NATIVE_WINDOW_FORMAT, &format);
    if (ret != 0)
    {
        ALOGE("Window has been abandoned! return value %d \n", ret);
        return gcvFALSE;
    }
    ...
    )
{
    ...
    Info->logicals[0] = (gctPOINTER) ANDROID_DUMMY;
    Info->physicals[0] = ANDROID_DUMMY;
    ...
    /* Even for compositor, the FB is wrapped in gralloc. */
    Info->isCompositor = gcvFALSE;
    ...
    )
{
    ...
    {
        rel = Window->dequeueBuffer_DEPRECATED(Window, &buffer);
    ...
    if (Window->lockBuffer_DEPRECATED(Window, buffer))
    {
        ALOGE("Failed to lock the window buffer");

#if ANDROID_SDK_VERSION >= 9
        /* Cancel this buffer. */
        Window->cancelBuffer_DEPRECATED(Window, buffer);
#else
        /* Queue back. */
