...
struct _gcoBUFFER
{
    ...
    /* Size of buffer. */
    gctSIZE_T                   size;
    gctSIZE_T                   maxSize[gcdMAX_CMD_BUFFERS];
...
Re_Construct2:
                    gcmERR_BREAK(gcoCMDBUF_Construct(
                        gcvNULL,
                        gcvNULL,
                        Buffer->maxSize[Buffer->count],
                        &Buffer->info,
                        &Buffer->commandBuffers[Buffer->count]));
                        if(status != gcvSTATUS_OK)
                        {
                            if( Buffer->maxSize[Buffer->count] <= (4<<10))
                            {
                                break;
                            }
                            Buffer->maxSize[Buffer->count] =  Buffer->maxSize[Buffer->count] >> 1;
                            goto Re_Construct2;
                        }
...
Re_Construct1:
        ...
        if(status != gcvSTATUS_OK)
        {
            if(buffer->maxSize[i] <= (4<<10))
            {
                goto OnError;
            }
            buffer->maxSize[i] = buffer->maxSize[i] >> 1;
            goto Re_Construct1;
        }
    ...
    )
{
    ...
    if (bytes > current->free)
    {
        ...
        if (bytes > Buffer->maxSize[Buffer->currentCommandBufferIndex] - Buffer->totalReserved)
        {
            /* This just won't fit! */
