    ...
    )
{
    ...
    */
    ...
    if (Context->hasComposition)
    {
        ...
        if (framebuffer == NULL)
        {
            ...
            {
                /* YUV framebuffer? */
                framebuffer->alignMask = YUV_BUFFER_ALIGN
            ...
            {
                /* RGB framebuffer. */
                framebuffer->alignMask = RGB_BUFFER_ALIGN
                                         / framebuffer->bytesPerPixel
                                       - 1;
            }
            ...
            /* Logs. */
            ALOGI("  Framebuffer(%p) detected: format=%d %dx%d",
                 framebuffer, framebuffer->format, xres, yres);

            ALOGI("  Framebuffer buffer(%p) detected: physical=0x%08x",
        ...
        if (target->physical != (gctUINT32) handle->phys)
        {
            ...
            else
            {
                ...
                hwcBuffer * b = framebuffer->head;

                while (b != framebuffer->head->prev)
                {
    ...
    if (Context->hasComposition)
    {
        ...
        for (gctUINT32 i = 0; i < List->numHwLayers; i++)
        {
            ...
            if (layer->source != gcvNULL)
            {
                ...
                /* Update source. */
                layer->source = handle->surface != 0 ? (gcoSURF) handle->surface
                              : (gcoSURF) handle->resolveSurface;
                ...
                gcmVERIFY_OK(
                    gcoSURF_GetFormat(layer->source, &type, &format));

                /* Get source surface tiling.
                 * TODO: Replace it with gcoSURF_GetTiling() */
                tiling = _TranslateTiling(type);
    ...
    */
    ...
    if (Context->hasOverlay)
    {
        ...
        {
            ALOGE("%s(%d): failed in setting overlay", __FUNCTION__, __LINE__);
        }
    }
    ...
    )
{
    ...
    /* Get multi-source blit alignment mask. */
    layer->alignMask = (layer->yuv ? YUV_BUFFER_ALIGN : RGB_BUFFER_ALIGN)
                       / layer->bytesPerPixel
                     - 1;
